            ...
            <table class="min-w-[1200px] divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right font-semibold text-slate-600">TVA %</th>
                ...
                <tbody id="items-body" class="divide-y divide-slate-100">
                    @foreach($oldItems as $index => $item)
                        <tr class="item-row">
                            ...
                            <td class="px-4 py-3"><input type="number" step="any" min="0.001" name="items[{{ $index }}][quantity]" value="{{ $item['quantity'] ?? 1 }}" class="quantity-input w-24 rounded-xl border border-slate-300 px-3 py-2 text-right text-sm" required><p class="stock-warning mt-1 hidden text-xs text-amber-700"></p></td>
                            <td class="px-4 py-3"><input type="number" step="0.01" min="0" name="items[{{ $index }}][unit_price]" value="{{ $item['unit_price'] ?? 0 }}" class="price-input w-32 rounded-xl border border-slate-300 px-3 py-2 text-right text-sm"></td>
                            <td class="px-4 py-3"><input type="number" step="0.01" min="0" max="100" name="items[{{ $index }}][discount_rate]" value="{{ $item['discount_rate'] ?? 0 }}" class="discount-input w-24 rounded-xl border border-slate-300 px-3 py-2 text-right text-sm"></td>
                            <td class="px-4 py-3"><input type="number" step="0.01" min="0" max="100" name="items[{{ $index }}][tax_rate]" value="{{ $item['tax_rate'] ?? $defaultTaxRate }}" class="tax-input w-24 rounded-xl border border-slate-300 px-3 py-2 text-right text-sm"></td>
...
function selectProduct(row, selectedProduct) {
    ...
    if (ref?.sale_price) row.querySelector('.price-input').value = ref.sale_price;
    if (ref?.discount_rate) row.querySelector('.discount-input').value = ref.discount_rate;
    refreshRow(row);
}
...
function refreshRow(row) {
    ...
    const taxRate = Number(row.querySelector('.tax-input').value || 0);
    ...
    row.querySelector('.line-detail').textContent = `HT ${money(ht)} - TVA ${money(tax)}`;
    refreshTotals();
}
...
function refreshTotals() {
    ...
    body.querySelectorAll('.item-row').forEach(row => {
        ...
        const t = Number(row.querySelector('.tax-input').value || 0);
        const lineGross = q * p;
...
function bind(row) {
    ...
    search.addEventListener('input', () => {
        ...
        results.innerHTML = matches.length
            ? matches.map(item => `<button type="button" class="block w-full border-b border-slate-100 bg-white px-3 py-2 text-left hover:bg-[#EAF4FB]" data-id="${item.id}"><strong>${item.internal_reference || item.code}</strong> - ${item.name}</button>`).join('')
            : '<div class="px-3 py-2 text-sm text-slate-500">Aucun article trouve.</div>';
        results.className = 'product-results absolute left-0 top-full z-30 mt-1 max-h-56 w-full min-w-80 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl';
    ...
    row.querySelectorAll('.quantity-input,.price-input,.discount-input,.tax-input').forEach(input => input.addEventListener('input', () => refreshRow(row)));
...
document.getElementById('add-line').addEventListener('click', () => {
    ...
    clone.querySelectorAll('input').forEach(input => input.value = input.classList.contains('quantity-input') ? 1 : (input.classList.contains('tax-input') ? @json($defaultTaxRate) : ''));
