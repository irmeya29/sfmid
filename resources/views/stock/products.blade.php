            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($products as $product)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-5 py-4 text-right font-bold @if($stockColumn === 'physical_stock') text-slate-950 @endif">{{ \App\Support\NumberFormatter::quantity($product->physical_stock) }}</td>
                        <td class="px-5 py-4 text-right font-bold @if($stockColumn === 'reserved_stock') text-slate-950 @endif">{{ \App\Support\NumberFormatter::quantity($product->reserved_stock) }}</td>
                        <td class="px-5 py-4 text-right font-bold @if($stockColumn === 'suspense_stock') text-slate-950 @endif">{{ \App\Support\NumberFormatter::quantity($product->suspense_stock) }}</td>
                        <td class="px-5 py-4 text-right font-bold @if($stockColumn === 'tool_stock') text-slate-950 @endif">{{ \App\Support\NumberFormatter::quantity($product->tool_stock) }}</td>
